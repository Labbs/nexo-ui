    ...
    <div className="w-full overflow-x-auto">
        ...
        <thead>
          <tr className="border-b">
            ...
            {schema.map((property, index) => {
              const Icon = propertyTypeIcons[property.type] || Type
                  ...
                  return (
                    ...
                    >
                      <div className="relative group/cell">
                        ...
                        />
                        {/* Open page button for title cell */}
                        {isTitle && (isHovered || isSelected) && !isEditing && (
                          <button
                            className="absolute right-1 top-1/2 -translate-y-1/2 h-6 px-2 flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground bg-background border rounded shadow-sm z-10"
                            onClick={(e) => {
                              e.stopPropagation()
                              onRowClick?.(row.id)
                            }}
      ...
      {rows.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
          ...
          <p className="text-sm mb-4">No rows yet</p>
          <Button
            variant="outline"
            size="sm"
            onClick={onAddRow}
