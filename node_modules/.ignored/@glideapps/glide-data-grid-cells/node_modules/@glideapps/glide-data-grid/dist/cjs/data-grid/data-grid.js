...
const DataGrid = (p, forwardedRef) => {
  ...
  const draw = React.useCallback(() => {
    ...
    (0, _dataGridRender.drawGrid)(canvas, {
      ...
      name
    }), getRowThemeOverride, drawCustomCell, drawHeader, prelightCells, imageLoader, lastBlitData, (_canBlit$current = canBlit.current) !== null && _canBlit$current !== void 0 ? _canBlit$current : false, damageRegion.current, hoverValues.current, hoverInfoRef.current, spriteManager, scrolling, enqueueRef.current);
  }, [width, height, cellXOffset, cellYOffset, translateX, translateY, columns, mappedColumns, enableGroups, freezeColumns, dragAndDropState, theme, headerHeight, groupHeaderHeight, selectedRows, disabledRows, rowHeight, verticalBorder, selectedColumns, isResizing, selectedCell, lastRowSticky, rows, getCellContent, getGroupDetails, getRowThemeOverride, drawCustomCell, drawHeader, prelightCells, imageLoader, spriteManager, scrolling]);
  ...
  const accessibilityTree = (0, _utils.useDebouncedMemo)(() => {
    ...
    return React.createElement("div", {
      ...
      role: "rowgroup"
    }, (0, _range.default)(cellYOffset, Math.min(rows, cellYOffset + accessibilityHeight)).map(row => React.createElement("div", {
      role: "row",
      key: row,
      "aria-rowindex": row + 2,
      "row-index": row + 2
  ...
  }, [width, mappedColumns, cellXOffset, dragAndDropState, translateX, rows, cellYOffset, accessibilityHeight, selectedCell === null || selectedCell === void 0 ? void 0 : selectedCell.cell, focusElement, getCellContent, canvasRef, onKeyDown, getBoundsForItem, onCellFocused], 200);
