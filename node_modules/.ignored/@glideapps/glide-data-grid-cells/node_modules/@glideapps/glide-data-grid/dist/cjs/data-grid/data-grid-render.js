...
var _colorParser = require("./color-parser");
...
function drawCell(ctx, row, cell, col, x, y, w, h, highlighted, theme, drawCustomCell, imageLoader, spriteManager, hoverAmount, hoverInfo, frameTime, lastToken, enqueue) {
  let hoverX;
  let hoverY;
  ...
  const needsAnim = (0, _dataGridLib.drawWithLastUpdate)(args, cell.lastUpdated, frameTime, forcePrep => {
    const drawn = (0, _dataGridTypes.isInnerOnlyCell)(cell) ? false : (drawCustomCell === null || drawCustomCell === void 0 ? void 0 : drawCustomCell({
      ctx,
      cell,
      theme,
      rect: {
    ...
    if (!drawn && cell.kind !== _dataGridTypes.GridCellKind.Custom) {
      var _r$renderDeprep;
      ...
      r.render(args);
      result = (_r$renderDeprep = r === null || r === void 0 ? void 0 : r.renderDeprep) !== null && _r$renderDeprep !== void 0 ? _r$renderDeprep : r;
    }
  });
...
function blitLastFrame(ctx, canvas, last, cellXOffset, cellYOffset, translateX, translateY, lastRowSticky, width, height, rows, totalHeaderHeight, dpr, columns, effectiveCols, getRowHeight) {
  ...
  let stickyWidth = (0, _dataGridLib.getStickyWidth)(effectiveCols);
...
function drawGridLines(ctx, effectiveCols, cellYOffset, translateX, translateY, width, height, drawRegions, spans, groupHeaderHeight, totalHeaderHeight, getRowHeight, verticalBorder, lastRowSticky, rows, theme) {
  var _theme$horizontalBord;

  let verticalOnly = arguments.length > 16 && arguments[16] !== undefined ? arguments[16] : false;
  ...
  walkGroups(effectiveCols, width, translateX, groupHeaderHeight, (span, groupName, x, y, w, h) => {
    ...
    if (group !== undefined) {
      ...
      if (group.actions !== undefined && isHovered) {
        ...
        const trans = (0, _colorParser.withAlpha)(fillColor, 0);
...
function drawHeader(ctx, x, y, width, height, c, selected, theme, isHovered, hasSelectedCell, hoverAmount, spriteManager, drawHeaderCallback) {
  ...
  if (isHovered && c.hasMenu === true && width > 35) {
    ...
    const trans = (0, _colorParser.withAlpha)(fillStyle, 0);
...
function drawCells(ctx, effectiveColumns, allColumns, height, totalHeaderHeight, translateX, translateY, cellYOffset, rows, getRowHeight, getCellContent, getGroupDetails, getRowThemeOverride, selectedRows, disabledRows, lastRowSticky, drawRegions, damage, selectedCell, selectedColumns, prelightCells, drawCustomCell, imageLoader, spriteManager, hoverValues, hoverInfo, outerTheme, enqueue) {
  ...
  walkColumns(effectiveColumns, cellYOffset, translateX, translateY, totalHeaderHeight, (c, drawX, colDrawStartY, clipX, startRow) => {
    ...
    walkRowsInCol(startRow, colDrawStartY, height, rows, getRowHeight, lastRowSticky, (drawY, row, rh, isSticky) => {
      ...
      if (cell.span !== undefined) {
        ...
        if (!handledSpans.has(spanKey)) {
          ...
          if (area !== undefined) {
            ...
            ctx.restore();
            if (typeof lastToken === "function") lastToken({
              ctx
            });
            lastToken = undefined;
            ctx.save();
      ...
      const theme = cell.themeOverride === undefined && rowTheme === undefined ? colTheme : { ...colTheme,
        ...rowTheme,
        ...cell.themeOverride
      };
      ctx.beginPath();
      const isFocused = (0, _dataGridLib.cellIsSelected)([c.sourceIndex, row], cell, selectedCell) || (0, _dataGridLib.cellIsInRange)([c.sourceIndex, row], cell, selectedCell);
      ...
      const highlighted = spanIsHighlighted || isFocused || !isSticky && (rowSelected || selectedColumns.hasIndex(c.sourceIndex));
      let fill;

      if (isSticky || theme.bgCell !== outerTheme.bgCell) {
        fill = (0, _colorParser.blend)(theme.bgCell, fill);
        if (typeof lastToken === "function") lastToken({
          ctx
        });
        lastToken = undefined;
      ...
      if (highlighted || rowDisabled) {
        if (rowDisabled) {
          fill = (0, _colorParser.blend)(theme.bgHeader, fill);
        }

        if (highlighted) {
          fill = (0, _colorParser.blend)(theme.accentLight, fill);
        }
        ...
        if ((prelightCells === null || prelightCells === void 0 ? void 0 : prelightCells.some(pre => pre[0] === c.sourceIndex && pre[1] === row)) === true) {
          fill = (0, _colorParser.blend)(theme.bgSearchResult, fill);
          if (typeof lastToken === "function") lastToken({
            ctx
          });
          lastToken = undefined;
      ...
      if (cellWidth > 10 && !skipContents) {
        ...
        const drawResult = drawCell(ctx, row, cell, c.sourceIndex, cellX, drawY, cellWidth, rh, highlighted, theme, drawCustomCell, imageLoader, spriteManager, (_hoverValue$hoverAmou = hoverValue === null || hoverValue === void 0 ? void 0 : hoverValue.hoverAmount) !== null && _hoverValue$hoverAmou !== void 0 ? _hoverValue$hoverAmou : 0, hoverInfo, frameTime, lastToken, enqueue);
...
function overdrawStickyBoundaries(ctx, effectiveCols, width, height, totalHeaderHeight, lastRowSticky, rows, getRowHeight, theme) {
  const drawX = (0, _dataGridLib.getStickyWidth)(effectiveCols);
  ctx.beginPath();
...
function drawFocusRing(ctx, width, height, cellYOffset, translateX, translateY, effectiveCols, allColumns, theme, totalHeaderHeight, selectedCell, getRowHeight, getCellContent, lastRowSticky, rows) {
  var _cell$span;
...
function drawGrid(canvas, buffers, width, height, cellXOffset, cellYOffset, translateX, translateY, columns, mappedColumns, enableGroups, freezeColumns, dragAndDropState, theme, headerHeight, groupHeaderHeight, selectedRows, disabledRows, rowHeight, verticalBorder, selectedColumns, isResizing, selectedCell, lastRowSticky, rows, getCellContent, getGroupDetails, getRowThemeOverride, drawCustomCell, drawHeaderCallback, prelightCells, imageLoader, lastBlitData, canBlit, damage, hoverValues, hoverInfo, spriteManager, scrolling, enqueue) {
  ...
  const drawHeaderTexture = () => {
    ...
    drawGridLines(overlayCtx, effectiveCols, cellYOffset, translateX, translateY, width, height, undefined, undefined, groupHeaderHeight, totalHeaderHeight, getRowHeight, verticalBorder, lastRowSticky, rows, theme, true);
  };
  ...
  if (selectedCell !== undefined && selectedCell.cell[0] === freezeColumns - 1) {
    drawFocusRing(targetCtx, width, height, cellYOffset, translateX, translateY, effectiveCols, mappedColumns, theme, totalHeaderHeight, selectedCell, getRowHeight, getCellContent, lastRowSticky, rows);
  }
  ...
  drawGridLines(targetCtx, effectiveCols, cellYOffset, translateX, translateY, width, height, drawRegions, spans, groupHeaderHeight, totalHeaderHeight, getRowHeight, verticalBorder, lastRowSticky, rows, theme);
  drawFocusRing(targetCtx, width, height, cellYOffset, translateX, translateY, effectiveCols, mappedColumns, theme, totalHeaderHeight, selectedCell, getRowHeight, getCellContent, lastRowSticky, rows);
...
function walkGroups(effectiveCols, width, translateX, groupHeaderHeight, cb) {
  ...
  for (let index = 0; index < effectiveCols.length; index++) {
    ...
    while (end < effectiveCols.length && (0, _dataGridLib.isGroupEqual)(effectiveCols[end].group, startCol.group) && effectiveCols[end].sticky === effectiveCols[index].sticky) {
